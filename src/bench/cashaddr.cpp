...
static void CashAddrEncode(benchmark::State &state) {
    ...
    while (state.KeepRunning()) {
        cashaddr::Encode("blackcoinsmattercoin", buffer);
    }
}

static void CashAddrDecode(benchmark::State &state) {
    const char *addrWithPrefix =
        "blackcoinsmattercoin:qprnwmr02d7ky9m693qufj5mgkpf4wvssv0w86tkjd";
    const char *addrNoPrefix = "qprnwmr02d7ky9m693qufj5mgkpf4wvssv0w86tkjd";
    while (state.KeepRunning()) {
        cashaddr::Decode(addrWithPrefix, "blackcoinsmattercoin");
        cashaddr::Decode(addrNoPrefix, "blackcoinsmattercoin");
    }
}
